...
"""
	Scenario-generation model with equiprobable scenarios
...
def build_model(EPwrAsVar: bool, DistAsVar: bool, ScaleVars: bool) -> AbstractModel:
	...
	def exp_pwr_formula(m, meas, pwr):
		# OBS: m.Sc must come first, because of https://github.com/Pyomo/pyomo/issues/31
		return 1 / m.Sc * sum(m.x[d] * m.Data[meas, d]**pwr for d in m.Days) * epwr_def_mult(m, meas, pwr)
	...
	def exp_xy_formula(m, meas1, meas2):
		# OBS: m.Sc must come first, because of https://github.com/Pyomo/pyomo/issues/31
		return 1 / m.Sc * sum(m.x[d] * m.Data[meas1, d] * m.Data[meas2, d] for d in m.Days) * cov_def_mult(m, meas1, meas2)
